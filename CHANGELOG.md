...
### Fixed
- Auto-enforce was failing the canary push every day at 4 AM whenever the Users template was enforced. Tachyon's write validator demands `system.users.*.password_hash` (a 34-char MD5-crypt-style field) when a Users template merge replaces the device's user list, even though the read endpoint never returns that field and a plain round-trip succeeds. Until we either compute device-side hashes or the firmware accepts plaintext replacements symmetrically, an enforce-driven Users push fails the dry-run every cycle — and because auto-enforce bundles all enforced templates into one push, the Users failure blocked the rest (NTP/SNMP/Watchdog/Syslog) too. Auto-enforce now skips templates whose `category` is in `AUTO_ENFORCE_UNSUPPORTED_CATEGORIES` (currently just `users`); compliance is still evaluated against the full applicable set so the Compliant/Non-Compliant chip stays honest, the unsupported categories just don't get pushed. The Config drawer's Users section gets a "Manual push only" badge with a tooltip explaining why. Operators can still push the Users template via the explicit Push to Devices flow when they need to rotate credentials
