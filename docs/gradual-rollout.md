...
- **Canary soak from fleet time.** The first widening past canary (`pct10`) is held until `canary_completed_at + firmware_canary_hold_days`, measured from when the canary phase actually completed on this fleet — **not** the firmware release date (which can be pre-cleared, giving zero real soak).
