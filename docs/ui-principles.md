...
- **Three buckets, three colors, three labels** — `green/yellow/red` → **Survives local storms / Survives heavy rain / May drop in heavy rain**. Labels are climate-relative: green = survives the worst storm the auto-detected region sees (the 99.99% rain rate), yellow = survives heavy rain (99.9%) but not that storm, red = below the local heavy-rain line. When a link lacks the inputs for a rain estimate (missing distance/channel), the tier falls back to raw signal strength and the popover badge says **"Signal only — no rain estimate"** rather than claiming rain resilience. The popover is the per-device source of truth. The aggregate surfaces — the Signal filter menu and chips, the AP/switch row dots (worst-of-children), and the chart legend tally — group by the bare bucket only, so in the rare fallback case a signal-only link can still appear under a rain word there; this is an accepted limitation since those surfaces have no per-device rain flag and the popover resolves it on hover. Tokens: `--success`, `--warning`, `--danger-bright` with `*-bg` and `*-text` variants.
